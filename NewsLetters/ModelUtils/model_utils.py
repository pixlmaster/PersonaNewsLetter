...
from NewsLetters.consts import ERROR_INVALID_JSON, ERROR_TOPIC_NOT_EXIST, ERROR_MISSING_FIELD, ERROR_DATA_INTEGRITY, \
    ERROR_ONLY_POST_ALLOWED, METHOD_POST, JSON_TOPIC_KEY, STR_ERROR
...
def handle_post_request(request, create_function):
    if request.method == METHOD_POST:
        try:
            ...
            return JsonResponse({STR_ERROR: ERROR_MISSING_FIELD.format(e)}, status=400)
        except ValueError as e:
            return JsonResponse({STR_ERROR: str(e)}, status=400)
        except IntegrityError:
            return JsonResponse({STR_ERROR: ERROR_DATA_INTEGRITY}, status=400)
        except Exception as e:
            return JsonResponse({STR_ERROR: str(e)}, status=500)
    else:
        return JsonResponse({STR_ERROR: ERROR_ONLY_POST_ALLOWED}, status=405)
